...
void Indexer::WriteTermHeader(const Tuple* tuple,
  ...
  file->write(reinterpret_cast<const char*>(&tuple->frequency),
              sizeof(unsigned));
  cout << endl << 't' << tuple->term << " c" << 0 << " d" << tuple->document
       << " f" << tuple->frequency;
}
...
void Indexer::WriteTuple(const Tuple* tuple,
                         std::unique_ptr<std::fstream>& file) {
  // Lazy initialization.
